...
import { addToLiked, deleteFromLiked, deleteFromWatchLater, addToWatchLater } from "../../apiCalls/index";
...
export function PlayVideo() {
  ...
  const presentInLiked = likedData.find(video => video._id === clickedVideo._id)




  const presentInWatchLater = watchLaterData.find(video => video._id === clickedVideo._id)
        ...
        <div className="play_video_content_container">
          ...
          <div className="play_video_content">
            ...
            <div className="play_video_icons">
              <AiFillLike title="Like" className={presentInLiked ? "present" : "absent"} onClick={() => {
                token ? null ? deleteFromLiked(clickedVideo._id, token, dataDispatch) :
                  addToLiked(clickedVideo, token, dataDispatch) : navigate("/login")
              }} />

              <BsFillStopwatchFill title="Watch later" className={presentInWatchLater ? "present" : "absent"}
                onClick={() => {
                  token ? presentInWatchLater ? deleteFromWatchLater(clickedVideo._id, token, dataDispatch) :
                    addToWatchLater(clickedVideo, token, dataDispatch) : navigate("/login")
                }} />

              <AiFillPlayCircle title="Playlist" />
            </div>
          </div>
        </div>
